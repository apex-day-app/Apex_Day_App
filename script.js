...
async function autoRegister() {
    ...
    if (!tgUser) {
        document.getElementById('loading-screen').innerHTML = '<h2>❌ Telegram से खोलें</h2><button onclick="window.location.href=\'https://t.me/APEX_DAY_bot\'">बॉट पर जाएं</button>';
        return;
    }
    ...
    try {
        ...
        if (!doc.exists) {
            userData = {
                uid: fbUser.uid, telegramId: tgUser.id.toString(),
        ...
        document.getElementById('user-telegram-id').innerText = '@' + (userData.username || 'user');
...
function closeAddBankPopup() { document.getElementById('add-bank-popup').classList.add('hidden'); }
...
function addBankAccount() {
    ...
    if (!bank || !holder || !number || !ifsc) return alert('सब भरें');
    
    users.doc(currentUser.uid).update({
        bank_accounts: firebase.firestore.FieldValue.arrayUnion({ bank, holder, number, ifsc })
    }).then(() => { closeAddBankPopup(); alert('बैंक अकाउंट जुड़ गया'); loadBankAccounts(); });
}

function loadBankAccounts() {
    if (!userData?.bank_accounts?.length) {
        document.getElementById('bank-options').innerHTML = '<p>कोई बैंक अकाउंट नहीं</p>';
        return;
    }
    let html = '';
    userData.bank_accounts.forEach((b, i) => {
        html += `<div class="bank-option" onclick="selectBank(${i})">${b.bank} - ****${b.number.slice(-4)}</div>`;
    });
    document.getElementById('bank-options').innerHTML = html;
}

function selectBank(i) { selectedBank = i; }
...
function submitAddMoneyRequest() {
    ...
    if (!amt || amt < 10 || !utr) return alert('राशि और UTR डालें');
...
function submitWithdrawRequest() {
    ...
    if (!amt || amt < 100 || amt > userData.balance) return alert('सही राशि डालें');
    if (selectedBank === null) return alert('बैंक अकाउंट चुनें');
...
function showWithdrawPage() {
    if (!userData?.bank_accounts?.length) return alert('पहले बैंक अकाउंट जोड़ें');
    loadBankAccounts();
...
function startGame() {
    ...
    function update(now) {
        if (now - last >= 10) {
            ...
            if (time <= 0) { document.getElementById('timer').innerText = '00:00'; endRound(); return; }
    ...
    users.doc(currentUser.uid).update({ balance: firebase.firestore.FieldValue.increment(-amt) })
        .then(() => {
            ...
            if (betMap[color][uid]) {
                ...
                item.className = 'bet-item'; item.id = `${color}-bet-${uid}`;
                item.innerHTML = `<div><span>${name}</span></div> <span>₹${amt}</span>`;
                list.insertBefore(item, list.firstChild);
            }
            userBets.add({ userId: uid, telegramId: userData.telegramId, userName: name, amount: amt, color, timeLeft: game.time, timestamp: new Date(), result: 'Pending' });
...
function logout() { auth.signOut().then(() => window.location.href = 'https://t.me/APEX_DAY_bot'); }
